...
const BOOTNODES = require('cse-common').bootstrapNodes.filter((node) => {
...
rlpx.on('peer:added', (peer) => {
  ...
  const cse = peer.getProtocols()[0]
  const requests = { headers: [], bodies: [], msgTypes: {} }

  const clientId = peer.getHelloMessage().clientId
  console.log(chalk.green(`Add peer: ${addr} ${clientId} (cse${cse.getVersion()}) (total: ${rlpx.getPeers().length})`))
  ...
  cse.once('status', () => {
    cse.sendMessage(devp2p.CSE.MESSAGE_CODES.GET_BLOCK_HEADERS, [ CHECK_BLOCK_NR, 1, 0, 0 ])
    forkDrop = setTimeout(() => {
  ...
  cse.on('message', async (code, payload) => {
    ...
    switch (code) {
      case devp2p.CSE.MESSAGE_CODES.NEW_BLOCK_HASHES:
        ...
        for (let item of payload) {
          ...
          setTimeout(() => {
            cse.sendMessage(devp2p.CSE.MESSAGE_CODES.GET_BLOCK_HEADERS, [ blockHash, 1, 0, 0 ])
            requests.headers.push(blockHash)
          }, ms('0.1s'))
        }
        break

      case devp2p.CSE.MESSAGE_CODES.TX:
        if (!forkVerified) break
      ...
      case devp2p.CSE.MESSAGE_CODES.GET_BLOCK_HEADERS:
        const headers = []
        // hack
        ...
        if (requests.headers.length === 0 && requests.msgTypes[code] >= 8) {
          ...
          cse.sendMessage(devp2p.CSE.MESSAGE_CODES.BLOCK_HEADERS, headers)
        }
        break

      case devp2p.CSE.MESSAGE_CODES.BLOCK_HEADERS:
        if (!forkVerified) {
          if (payload.length !== 1) {
          ...
          while (requests.headers.length > 0) {
            ...
            if (header.hash().equals(blockHash)) {
              isValidPayload = true
              setTimeout(() => {
                cse.sendMessage(devp2p.CSE.MESSAGE_CODES.GET_BLOCK_BODIES, [ blockHash ])
      ...
      case devp2p.CSE.MESSAGE_CODES.GET_BLOCK_BODIES:
        if (requests.headers.length === 0 && requests.msgTypes[code] >= 8) {
          peer.disconnect(devp2p.RLPx.DISCONNECT_REASONS.USELESS_PEER)
        } else {
          cse.sendMessage(devp2p.CSE.MESSAGE_CODES.BLOCK_BODIES, [])
        }
        break

      case devp2p.CSE.MESSAGE_CODES.BLOCK_BODIES:
        if (!forkVerified) break
      ...
      case devp2p.CSE.MESSAGE_CODES.NEW_BLOCK:
        if (!forkVerified) break
      ...
      case devp2p.CSE.MESSAGE_CODES.GET_NODE_DATA:
        if (requests.headers.length === 0 && requests.msgTypes[code] >= 8) {
          peer.disconnect(devp2p.RLPx.DISCONNECT_REASONS.USELESS_PEER)
        } else {
          cse.sendMessage(devp2p.CSE.MESSAGE_CODES.NODE_DATA, [])
        }
        break

      case devp2p.CSE.MESSAGE_CODES.NODE_DATA:
        break

      case devp2p.CSE.MESSAGE_CODES.GET_RECEIPTS:
        if (requests.headers.length === 0 && requests.msgTypes[code] >= 8) {
          peer.disconnect(devp2p.RLPx.DISCONNECT_REASONS.USELESS_PEER)
        } else {
          cse.sendMessage(devp2p.CSE.MESSAGE_CODES.RECEIPTS, [])
        }
        break

      case devp2p.CSE.MESSAGE_CODES.RECEIPTS:
        break
    }
  })
})
