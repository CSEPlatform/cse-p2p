...
const debug = createDebugLogger('devp2p:cse')
...
class CSE extends EventEmitter {
  ...
  static cse62 = { name: 'cse', version: 62, length: 8, constructor: CSE }
  static cse63 = { name: 'cse', version: 63, length: 17, constructor: CSE }
  ...
  _handleMessage (code, data) {
    ...
    switch (code) {
      ...
      case MESSAGE_CODES.NEW_BLOCK:
        if (this._version >= CSE.cse62.version) break
        return
      ...
      case MESSAGE_CODES.RECEIPTS:
        if (this._version >= CSE.cse63.version) break
        return
  ...
  sendStatus (status) {
    ...
    debug(`Send STATUS message to ${this._peer._socket.remoteAddress}:${this._peer._socket.remotePort} (cse${this._version}): ${this._getStatusString(this._status)}`)
